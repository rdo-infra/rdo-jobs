...
cp $TEMPEST_DIR/etc/tempest.conf $TEMPEST_LIST/tempest.conf
...
if [[ $(wc -l < "${TEMPEST_LIST}include.txt") -ge 0 ]]; then
    ...
    echo "tempest.api.compute" > "${TEMPEST_LIST}include.txt"
    echo "tempest.api.image.v2" > "${TEMPEST_LIST}include.txt"
    echo "tempest.api.network" > "${TEMPEST_LIST}include.txt"
    echo "tempest.api.object_storage" > "${TEMPEST_LIST}include.txt"
    echo "tempest.api.volume" > "${TEMPEST_LIST}include.txt"
fi
