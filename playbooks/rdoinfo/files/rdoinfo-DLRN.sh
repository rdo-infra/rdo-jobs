        ...
        if [ -n "$UPSTREAM_ID" ]; then
            ...
            if [ "${ZUUL_PIPELINE}" == "check" ]; then
                ...
                if [ -n "$REF" -a "$REF" != null ]; then
                    ...
                    echo "INFO: the upstream patch with ID $UPSTREAM_ID is fetched and will be built."
                    echo "$UPSTREAM_ID" >> /tmp/upstream_ids
                fi
                git log -1
                popd
            fi
